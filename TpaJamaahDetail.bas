...
Sub Activity_Create(FirstTime As Boolean)
	...
	MainScroll.Panel.LoadLayout("TpaJamaahDetail")
...
Sub Activity_Resume
	CodeSemua.ExecuteUrlGet(Main.server&"api/mosque/detail?id="&TpaJamaah.id_clicked,"LoadData",Me)
End Sub
...
Sub JobDone (Job As HttpJob)
	If Job.Success = True Then
		...
		Else
			...
			NavbarTitle.Text = "TPA/MDA "&mosque.Get("name")
...
Sub Button1_Click
	CodeSemua.ExecuteUrl(Main.server&"api/mosque/tpa/store?id="&TpaJamaah.id_clicked,"jamaah="&HomeJamaah.id&"&fund="&EditTotal.Text&"&account="&accountId&"&public="&publicId, "Store",Me)
End Sub
